...
type Dataset<T> = {
    data: T[],
    error: string
    get: <G>(query: string, index?: number) => G,
    first: () => T | null,
    ok: () => boolean
    errorMessage: () => string
}
...
dataset.prototype.ok = function () {
    return this.data.length != 0 && this.data[0] != null && !this.error
}
dataset.prototype.get = function <G>(query, index = 0): G | G[] | null {
    if (index == -1)
        return this.data?.map(item => { return item[query] }) || null//.filter(item => !!item) || null
